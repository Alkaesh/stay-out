...
| Что | Зачем |
|---|---|
| Visual Studio 2022 (Desktop C++ + CMake) | Сборка research-DLL |
| MSBuild (ставится с VS Build Tools) | Сборка `injector2` (loader.exe + драйвер) |
| **WDK 10.0.26100.x** + matching SDK | Сборка драйвера `modern_injector.sys` (без него только loader) |
| **Test signing:** `bcdedit /set testsigning on` + ребут | Загрузка самоподписанного `.sys` |
| Игра **Stay Out** (клиент `sogame.exe`, x64) | Цель инжекта |
| Запуск **от администратора** | Установка/старт kernel-драйвера, открытие `\\.\ModernInjector` |
...
`build.bat` делает две вещи разными системами сборки:

1. **Research-DLL** — CMake + Ninja (`stayout_dll`).
2. **injector2** — MSBuild по `injector2\kernelmode.sln` (два проекта):
   - `kernelmode.vcxproj` → `loader.exe` (user-mode, всегда)
   - `modern_injector.vcxproj` → `modern_injector.sys` (driver)
   `build.bat` сам детектит WDK (по наличию `ntifs.h`): если WDK есть —
   собирается весь solution (loader + driver); нет — только `loader.exe`.
   `test_client.exe` — опционально (`/p:BuildTestClient=true`).

**Результат:**
```
build\stayout_dll.dll
injector2\x64\Release\loader.exe
injector2\x64\Release\modern_injector.sys   ← нужен WDK
injector2\x64\Release\test_client.exe        ← опционально (/p:BuildTestClient=true)
```
...
> **Драйвер `modern_injector.sys` полностью собирается** под установленным
> WDK (проверено: PE32+, x64, Subsystem=Native, entry=DriverEntry, импортирует
> ntoskrnl.exe). Проект использует обычный MSVC **v143** toolset + явные пути к
> WDK kernel-headers/libs (`Include/<ver>/km`, `Lib/<ver>/km/x64`) —
> **без** VSIX-интеграции WDK (`WindowsKernelModeDriver10.0` toolset не нужен).
> Если у тебя другая версия WDK — поправь `WDKVer` в `modern_injector.vcxproj`.
>
> `kernelmode.sln` теперь содержит **два** проекта:
> - `kernelmode.vcxproj` → `loader.exe` (user-mode, всегда)
> - `modern_injector.vcxproj` → `modern_injector.sys` (driver, нужен WDK)
>
> ⚠️ Драйвер **скомпилирован и слинкован, но НЕ протестирован в рантайме**
> (загрузка через `sc`/test-signing). Kernel-код (PEB-walking, undocumented
> `RtlCreateUserThread`) может потребовать отладки при первом реальном запуске.
> Все предположения задокументированы инлайн в `modern_injector.cpp`.
...
## 3. Инжект (kernel-mode)

> Это **kernel-mode** флоу: инжект делает драйвер `modern_injector.sys` из
> ring 0, а не user-mode `CreateRemoteThread`. Поэтому перед `run.bat`
> драйвер должен быть собран (см. п.1, нужен WDK) и загружен.

### 3.0. Один раз: test signing + сборка драйвера

```
bcdedit /set testsigning on
```
и **перезагрузка** (без этого `sc start ModernInjector` упадёт). Затем
собрать `.sys` через `build.bat` (WDK должен быть установлен).
...
`run.bat` делает по порядку:
1. Проверяет права админа.
2. Находит `stayout_dll.dll` и `injector2\x64\<cfg>\loader.exe`.
3. Резолвит PID цели (число из аргумента, иначе `tasklist` по `sogame.exe`).
4. Гарантирует, что сервис драйвера установлен и запущен:
   - `sc query ModernInjector` — если сервиса нет:
     `sc create ModernInjector binPath= "<abs>\modern_injector.sys" type= kernel`
   - `sc start ModernInjector` (если STOPPED).
5. Вызывает `loader.exe <PID> <abs_dll_path>` — он шлёт `IOCTL_INJECT_DLL`
   в драйвер. Полный вывод пишется в `%TEMP%\inject_log.txt`.

**Ожидаемый вывод loader'а:**
```
[*] Requesting injection: PID=12345 DLL=...\stayout_dll.dll
DLL injection requested successfully!
```
(Сам маппинг и логи DLL см. в п.4 — консоль «[Stay Out] BigWorld Research
Console» и `%TEMP%\stayout_log.txt`.)

### 3.2. Типичные проблемы именно kernel-flow

- `Failed to open driver: 2` — сервис не создан/не запущен (`run.bat`
  обычно делает это сам; проверь `sc query ModernInjector`).
- `sc start` падает — test signing не включён, либо `.sys` собран с
  mismatched WDK/SDK, либо драйвер крашится при загрузке (Event Viewer →
  System).
- `IOCTL failed: 5` — драйвер загружен, но `DeviceControl` отклонил
  запрос (например, целевой PID не найден ядром: `PsLookupProcessByProcessId`).
...
- SEH/CRT: kernel manual-map без регистрации `.pdata` может крашить DLL с CRT.
  Текущая DLL почти не использует CRT, но если добавишь — потребуется
  доработать `modern_injector.cpp` (регистрация `.pdata` через
  `RtlAddFunctionTable` из драйвера — пока не реализована).
...
| Файл | Назначение |
|---|---|
| `build.bat` | Сборка DLL (CMake/Ninja) + injector2 (MSBuild). |
| `run.bat` | Инсталляция/старт драйвера + инжект в `sogame.exe` от админа. |
| `injector2/loader.cpp` | User-mode loader: шлёт IOCTL в драйвер. Принимает `<PID> <dll>`. |
| `injector2/modern_injector.cpp` | Kernel-драйвер `.sys` (маппинг DLL из ring 0). **Пока не линкуется** — см. п.1. |
| `config.hpp` | Все настройки игры в одном месте. |
| `diagnostics.cpp` | Дампы `dir()` для RE атрибутов. |
| `%TEMP%\inject_log.txt` | Лог вывода loader'а (перезаписывается каждый запуск). |
| `%TEMP%\stayout_log.txt` | Полный лог DLL (перезаписывается каждый инжект). |
